...
describe('Snapshot-Based Regression Tests', () => {
    ...
    describe('Algorithm Output Snapshots', () => {
        Object.entries(SNAPSHOT_CONFIGURATIONS).forEach(([_key, config]) => {
                ...
                const snapshotData = {
                    ...
                    metrics: {
                        ...
                        amplification: Number(
                            (data.actualCells.size / data.requestedCells.size).toFixed(4)
                        ),
                    },
            ...
            algorithms.forEach(cellAlg => {
                algorithms.forEach(chunkAlg => {
                    ...
                    comparisonMatrix[key] = {
                        ...
                        coalescing: Number(
                            (data.touchedChunks.size / data.chunkedRanges.length).toFixed(4)
                        ),
                        ranges: data.chunkedRanges.length,
                        efficiency: Number(
                            (100 / (data.actualCells.size / data.requestedCells.size)).toFixed(2)
                        ),
                    };
                });
            });
